  ...
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!db.objectStoreNames.contains(STORE_NAME)) {
        const store = db.createObjectStore(STORE_NAME, { keyPath: 'meetingId' });
...
export async function saveEvaluationResult(meetingId: number, results: any): Promise<void> {
...
export async function getEvaluationResult(meetingId: number): Promise<StoredEvaluationResult | null> {
  const db = await initDB();
  return new Promise((resolve, reject) => {
